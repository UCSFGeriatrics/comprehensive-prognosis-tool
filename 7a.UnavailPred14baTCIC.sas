/*********** Purpose: Apply Unavailable predictor method to final baTCIC model ***********/
...
%let NUMPRED=14; /*number of predictors WITHOUT including predictors that are forced in (e.g. ageint5, gender*/
%let NUMPREDPLUSONE=15; /*number of predictors defined in &NUMPRED (e.g.18) plus one*/
...
%macro best_TCIC;
	...
	*Get the TCIC=SBC of full models with 40 predictors;
	...
	/*Start with 16 predictors*/
	%let BASE=%sysfunc(compbl(ageint5 gender R5EATA R5CANCRE R5DIABE heart3g R5HIBPE R5MEALSA R5MONEYA H5LVALONE R5LUNGE R5PUSHA R5WALKSA bmi4g smoking3g stroke3g)); /*all 16 initial variables*/
    %let CLASS=%sysfunc(compbl(gender R5EATA R5CANCRE R5DIABE heart3g R5HIBPE R5MEALSA R5MONEYA H5LVALONE R5LUNGE R5PUSHA R5WALKSA bmi4g smoking3g stroke3g)); /*all 16-ageint5(continuous variable)=15 initial variables*/
    %let DELE=%sysfunc(compbl(R5EATA R5CANCRE R5DIABE heart3g R5HIBPE R5MEALSA R5MONEYA H5LVALONE R5LUNGE R5PUSHA R5WALKSA bmi4g smoking3g stroke3g)); /*all 16 variables-2(ageint5 gender)=14*/
	%let BASESEC=99.2; 
	/*99.2 is the initial total time cost when all 16 variables are in the model. As we go from one model to a smaller model the basesec variable will be replaced for a new basesec based on the variables left in the model*/

	/*Run PHREG for the 1st time with all 16 predictors in the model*/
	...
	data BASE_TCIC (keep= VARINMODEL DELEVAR DELELIST VARSEC &Cvars &IAUCvars &AICvars &TCICvars C_avg iauc_avg TCIC_avg); 
	...
	proc delete data=FITS1 FITS1_Cox CensUncens CensUncens_Cox concord concord_cox iauc iauc_cox estimates; run; quit;
    /*proc delete data=FITS1 CensUncens DF concord iauc estimates; run; quit;*/

   proc append base=TCIC data=BASE_TCIC force; run; /*TCIC is the final dataset with backward elimination steps from 38 variables to 2*/
   proc delete data=BASE_TCIC; run; quit;

	%do i=1 %to 1; /*%do i=1 %to &NUMPRED;*/
	  %DeleteOneVar;
	     data _null_;
	       set AVGCTABLE (keep=VARINMODEL VARINCLASS DELELIST VARSEC); /*set AVGCTABLE2 (keep=VARINMODEL VARINCLASS DELELIST VARSEC);*/
		...
		*/
		...
		proc append base=TCIC data=AVGCTABLE (drop=VARINCLASS); run;
...
/*
   proc sort data= AVGCTABLE tagsort; by descending TCIC_avg; run;
...
data outdata2.baTCIC_1UnavailPred; set TCIC (drop=DELELIST); run;
...
ods csv file="path\Results_baTCIC_1UnavailPred.csv";
proc print data=outdata2.baTCIC_1UnavailPred; run;
ods csv close;

ods listing; /* turn back on the output window / "listing" output*/

proc means data=outdata2.baTCIC_1UnavailPred stackodsoutput n mean clm stderr min max maxdec=4;
 var C_adl C_walk C_death C_avg iauc_adl iauc_walk iauc_death iauc_avg;
 ods output summary=outdata2.baTCIC_1UnavailPredSummary;
 where delevar ne "";
run;

ods csv file="path\Results_baTCIC_1UnavailPredSummary.csv";
proc print data=outdata2.baTCIC_1UnavailPredSummary; run;
ods csv close;
